...
LOG_PATH="$REPORT_DIR/train_baselines_${TIMESTAMP}.log"
LATEST_LOG_PATH="$REPORT_DIR/train_baselines.log"
...
{
  echo "Baseline training started at $(date '+%Y-%m-%d %H:%M:%S %z')"
  echo "command=${CMD[*]}"
  echo
  "${CMD[@]}"
  echo
  echo "Baseline training finished at $(date '+%Y-%m-%d %H:%M:%S %z')"
} 2>&1 | tee "$LOG_PATH"
